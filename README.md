...
In trust mode, `FINAL_ANSWER.*` is all-or-nothing. The runner-owned `analysis_review_status.recommendation_admissibility` status records `final_answer_recommendation_indices`, `partial_only_recommendation_indices`, `excluded_recommendation_indices`, and `reasons_by_recommendation_index` using only the canonical reasons `accepted_with_caveat`, `inferred_grounding`, `not_accepted`, and `topic_blocked`. `accept_with_caveat` and accepted-but-inferred recommendations are not final-admissible in trust mode. `PARTIAL_ANSWER.*` is the explicit scoped fallback: its candidate subset comes from the final-admissible plus partial-only recommendations, while whole-artifact promotion still reuses the existing partial gates together with provenance binding, global topic blockers, and minimum-threshold fallout checks.

The runner also records `analysis_review_status.publishability` with `final_answer_publishable` and ordered `blocking_causes`; when final publication is blocked, artifact selection falls through to `PARTIAL_ANSWER.*` when eligible, otherwise `BEST_DRAFT.*`.
