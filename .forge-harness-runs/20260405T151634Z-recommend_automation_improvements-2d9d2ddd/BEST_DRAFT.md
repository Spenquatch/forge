...
Release-watch jobs still redispatch already-open Codex/Claude Code candidates every night, snapshot workflows can collide because they lack per-version concurrency, and the Claude Code watcher omits the semver corruption guard and best-effort PR handling that protect the Codex automation flow.

## Recommendations

### 1. Skip release-watch dispatch when the automation branch or PR already exists (risk, high)
...
**Suggested change:** Before `createWorkflowDispatch`, call the GitHub API (branches or search issues) for `automation/<tool>-${version}`; if a branch or PR already exists, log and exit so only one automation run stays active per release.
...
### 2. Add per-version concurrency groups to both snapshot workflows (recommendation, medium)
...
**Suggested change:** Define `concurrency: group: ${{ github.workflow }}-${{ inputs.version }}, cancel-in-progress: false` (optionally a second `${{ github.workflow }}-latest` guard) near the top of both workflows so only one run per version proceeds while different versions can still run in parallel.
...
### 3. Make Claude Codes PR creation step best-effort like Codex to absorb branch push races (risk, medium)
...
**Suggested change:** Mirror Codex by adding `continue-on-error: true` (or catching the non-fast-forward exit code) around the Claude Code PR step and log a message pointing maintainers to update the existing automation PR instead of re-running the whole workflow.
...
### 4. Add a corruption guard for `latest_validated` in the Claude Code watcher (risk, medium)
...
**Suggested change:** Port the Codex guard: after reading `cli_manifests/claude_code/latest_validated.txt`, validate it with `parseStrictStableSemver` and call `core.setFailed` if parsing fails so the automation stops instead of redispatching on corrupt metadata.
